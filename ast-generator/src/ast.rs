...
pub mod expressions {
    ...
    pub fn define_ast(output_dir: &str, base_name: &str, types: &[&str]) -> std::io::Result<()> {
        ...
        for t in &tree_types {
            writeln!(file, "\t\t\t{}::{}(v) => v.accept({}_visitor),", base_name, t.base_name.to_lowercase(), base_name.to_lowercase())?;
        }
        writeln!(file, "\t\t}}")?;
        writeln!(file, "\t}}")?;
        writeln!(file, "}}\n")?;
        ...
        for t in &tree_types {
            ...
            writeln!(file, "\t\tvisitor.visit_{}_{}(self)", t.base_name.to_lowercase(), base_name.to_lowercase())?;
            writeln!(file, "\t}}")?;
            writeln!(file, "}}\n")?;
        }
